...
def ouverture(cursor,connexion,locker:str,utilisateur)->None:
    ...
    occupe:str = f"UPDATE casier SET occupe = FALSE WHERE (id_casier LIKE '{locker}')"
    cursor.execute(occupe)
    connexion.commit()
    h_restant:str = f"UPDATE casier SET heure_restant = NULL WHERE (id_casier LIKE '{locker}')"
    cursor.execute(h_restant)
    connexion.commit()
    h_reservation:str = f"UPDATE casier SET heure_reservation = NULL WHERE (id_casier LIKE '{locker}')"
    cursor.execute(h_reservation)
    connexion.commit()
    h_fermeture:str = f"UPDATE casier SET heure_fermeture = NULL WHERE (id_casier LIKE '{locker}')"
    cursor.execute(h_fermeture)
    connexion.commit()
    id:str = f"UPDATE casier SET id_uti = NULL WHERE (id_casier LIKE '{locker}')"
    cursor.execute(id)
    connexion.commit()
    verification:str = f"SELECT utilisation_en_cours FROM utilisateur WHERE (id_utilisateur LIKE '{utilisateur}')"
    value:int
    ...
    if(value != 0):
        value = value - 1
    nb_uti:str = f"UPDATE utilisateur SET utilisation_en_cours = {value} WHERE (id_utilisateur LIKE '{utilisateur}')"
    cursor.execute(nb_uti)
    connexion.commit()
...
def fermeture(cursor,connexion,locker:str,utilisateur:str) -> None:
    ...
    occupe: str = f"UPDATE casier SET occupe = TRUE WHERE (id_casier LIKE '{locker}')"
    cursor.execute(occupe)
    connexion.commit()
    id_uti: str = f"UPDATE casier SET id_uti = '{utilisateur}' WHERE (id_casier LIKE '{locker}')"
    cursor.execute(id_uti)
    connexion.commit()
    verification:str = f"SELECT utilisation_en_cours FROM utilisateur WHERE (id_utilisateur LIKE '{utilisateur}')"
    value:int
    cursor.execute(verification)
